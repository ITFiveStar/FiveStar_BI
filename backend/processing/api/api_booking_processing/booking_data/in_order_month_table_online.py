"""
Online deployment version of in_order_month_table.py
...
def process_in_order_month_booking(memory_processor):
    """
    Process in-order month booking data using memory processor for online deployment
    
    Args:
        memory_processor: MemoryFileProcessor instance containing uploaded files
    
    Returns:
        tuple: (Accrued_Adjusted_PnL, return_principal_to_inventory, PnL_month_str, PnL_month_month, PnL_month_year)
    """
    ...
    # [Continue with rest of the processing logic exactly as in original file...]
    # For brevity, I'll include key processing steps but you'll need the full logic

    # Return the key outputs needed by the journal entry script
    PnL_month_str = PnL_month.strftime('%Y-%m-%d')
    PnL_month_month = PnL_month.strftime('%B')
    PnL_month_year = PnL_month.strftime('%Y')
    ...
    statement_details_filtered = statement_details[statement_details['settlement-end-date-PST-PDT'] <= PnL_month]
    ...
    statement_details_filtered.fillna('Missing', inplace=True) # to ensure pivot, only category columns in statement_details will have blank value (e.g. non-order related amount rows)
    statement_details_filtered = statement_details_filtered[[
    ...
    statement_unique_order_ids = statement_details_pivoted[statement_details_pivoted['transaction-type'] == 'Order']['order-id'].dropna().unique()
    all_orders_PnL['Payment Status'] = all_orders_PnL['amazon-order-id'].apply(
        lambda x: 'Paid' if pd.isna(x) else ('Paid' if x in statement_unique_order_ids else 'Unpaid')
    )
    ...
    first_day_PnL_month = PnL_month.replace(day=1)
    Statement_nonRefund_PnL = sum_statement_items_nonReturn(statement_details_pivoted, PnL_month, first_day_PnL_month)

    # Project P&L Values based on all_orders_PnL during generating sku_PnL
    Project_PnL_toReconcile = all_orders_PnL.groupby('Payment Status')[columns_to_sum_all_orders_PnL].sum().T
    ...
    if 'Paid' in Project_PnL_toReconcile.columns:
        Project_PnL_toReconcile.rename(columns={'Paid': 'Project PnL Paid'}, inplace=True)
    else:
        Project_PnL_toReconcile['Project PnL Paid'] = np.nan
    if 'Unpaid' in Project_PnL_toReconcile.columns:
        Project_PnL_toReconcile.rename(columns={'Unpaid': 'Project PnL Unpaid'}, inplace=True)
    else:
    ...
    Project_PnL_toReconcile.loc[Project_PnL_toReconcile['Project PnL Items'].isin(columns_to_negate_sku_PnL), 
        ['Project PnL Paid', 'Project PnL Unpaid']] *= -1
    ...
    Project_PnL_toReconcile.loc[
        ...
        ['Project PnL Paid', 'Project PnL Unpaid']
    ] = [
        Project_PnL_toReconcile.loc[
            Project_PnL_toReconcile['Project PnL Items'] == 'Subscription Fee', 
            ['Project PnL Paid', 'Project PnL Unpaid']
        ].sum(axis=1).values[0], 0
    ]
    Project_PnL_toReconcile.loc[
    ...
    Statement_Project_PnL_Reconciliation['Statement Values'] = np.where(
        ...
        Statement_Project_PnL_Reconciliation['Project PnL Paid'] + Statement_Project_PnL_Reconciliation['Project PnL Unpaid'],
    ...
    Statement_Project_PnL_Reconciliation['Adjustment Items - Accrued Adjusted PnL to Adjust'] = Statement_Project_PnL_Reconciliation.apply(
        lambda row: (
            (row['Statement Values'] - row['Project PnL Paid']) 
    ...
    Statement_ReturnPnL = sum_statement_items_Return(statement_details_pivoted, PnL_month)
    ...
    Statement_ReturnPnL['Project PnL Paid'] = np.nan
    ...
    Accrued_Adjusted_PnL['Accrued Adjusted Values'] = Accrued_Adjusted_PnL.apply(
        lambda row: (
            (0 if pd.isna(row['Project PnL Paid']) else row['Project PnL Paid']) +
    ...
    return_principal_to_inventory = statement_details_pivoted[
        ((statement_details_pivoted['transaction-type'] == 'Refund') |
        (statement_details_pivoted['transaction-type'] == 'Chargeback Refund') |
        (statement_details_pivoted['transaction-type'] == 'A-to-z Guarantee Refund'))
        & 
        (statement_details_pivoted['all order PnL Date'] == PnL_month_str)
    ]['ItemPrice: Principal'].sum()
    
    return Accrued_Adjusted_PnL, return_principal_to_inventory, PnL_month_str, PnL_month_month, PnL_month_year
