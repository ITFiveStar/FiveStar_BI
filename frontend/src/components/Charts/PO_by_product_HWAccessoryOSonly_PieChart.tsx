...
const ProductPOPieChart: React.FC<{ 
  displayMode?: string; 
  dateUpTo?: Date | null;
  activeTab?: number;
}> = ({
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await axios.get(`/cogs_details/PO_pie_chart_data_product_in_hardware_accessory_os?${params.toString()}`);
      setRawData(res.data.pie_by_product);
      setTotal(res.data.total[valueType]);

      const processed = res.data.pie_by_product.map((d: PieDatum) => ({
        id: d.id,
        value: d[valueType],
      }));
      setData(processed);
  ...
  useEffect(() => {
    fetchData();
  }, [displayMode, dateUpTo, activeTab, valueType]);
...
export default ProductPOPieChart;
