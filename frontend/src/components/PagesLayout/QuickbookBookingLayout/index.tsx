...
import './QuickbookBookingLayout.css';
    ...
    steps: [
      {
        ...
        description: "to process and book journals for deposits during order month and AR AP estimates",
        path: "/automate-accounting/quickbooks/month-end"
      },
      {
        title: "Book Post Month-End Financials",
        description: "to process and book journals for deposits post order month with AR AP Closure and Adjustments",
        path: "/automate-accounting/quickbooks/statement-decomp"
      },
      {
        title: "Review Amazon Proceeds",
        description: "to visualize the detailed cash flow of Amazon Proceeds and track revenue and expenses flow",
        path: "#"
      }
    ]
  }
...
interface QuickbookBookingLayoutProps {
  children: React.ReactNode;
  pageTitle: string;
}

const QuickbookBookingLayout: React.FC<QuickbookBookingLayoutProps> = ({ children, pageTitle }) => {
  ...
  const getActiveStep = (): { sectionIndex: number, stepIndex: number } => {
    // Special case: if we're on the post-month-end page, we treat it the same as statement-decomp (step 2)
    if (currentPath === '/quickbooks/post-month-end') {
      // Find the section and step index for statement-decomp
      for (let i = 0; i < workflowSteps.length; i++) {
        const section = workflowSteps[i];
        for (let j = 0; j < section.steps.length; j++) {
          if (section.steps[j].path === '/quickbooks/statement-decomp') {
    ...
    // Normal case: find the exact path match
    for (let i = 0; i < workflowSteps.length; i++) {
      const section = workflowSteps[i];
      for (let j = 0; j < section.steps.length; j++) {
  ...
  return (
    <PageContainer className="qbbooking-workflow-container">
      {/* Custom Header */}
      ...
      <MainContent sx={{
        ...
        <RightColumn className="qbbooking-content-wrapper">
          <Typography variant="h4" sx={{ 
            color: '#47709B', 
            fontWeight: 700, 
            mb: 4,
...
export default QuickbookBookingLayout; 